...
describe("buildSealedRoom", () => {
  ...
  it("adds desk, chair, and one hotspot per command-file entry in the office stage", () => {
    ...
    expect(hotspots.map(h => h.userData.label).sort()).toEqual([
      "Labs",
      "Records",
    ]);
  });
  ...
  it("applies skin material maps supplied by the React shell", () => {
    const texture = new THREE.Texture();
    const texturedSkin: RoomSkin = {
      ...skin,
      materials: {
        floor: {
          color: "#332211",
          texture: "/brand/test-floor.svg",
          roughness: 0.91,
        },
      },
    };

    const g = buildSealedRoom("waiting", texturedSkin, { floor: texture });
    const floor = g.getObjectByName("floor") as THREE.Mesh;
    const mat = floor.material as THREE.MeshStandardMaterial;
    expect(mat.map).toBe(texture);
    expect(`#${mat.color.getHexString()}`).toBe("#332211");
    expect(mat.roughness).toBeCloseTo(0.91);
  });
  ...
  it("builds without throwing for the neutral skin (no brand, empty command file)", () => {
    ...
    expect(g.children.filter(c => c.name.startsWith("hotspot:"))).toHaveLength(
      0
    );
  });
});
